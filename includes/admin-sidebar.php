...
<!-- ─── SIDEBAR ─── -->
<aside id="adminSidebar" class="w-64 fixed inset-y-0 left-0 z-50 transform -translate-x-full lg:translate-x-0 lg:static lg:z-auto transition-transform duration-200 ease-in-out flex flex-col shrink-0 bg-[#005f73] border-r border-teal-700/40 shadow-xl text-white font-inter">
    <div class="h-16 flex items-center justify-between px-5 bg-teal-900/40 backdrop-blur-sm border-b border-teal-700/40">
        <div class="flex items-center gap-2">
            ...
            <span class="text-[10px] font-extrabold uppercase tracking-wider px-2 py-0.5 rounded-full bg-teal-600/40 text-teal-100 border border-teal-400/30">ADMIN</span>
        </div>
    ...
    <nav class="flex-1 py-4 space-y-1 px-3 overflow-y-auto scrollbar-thin">
        <?php foreach ($nav_items as $item): ?>
            ...
            <a href="<?= $item['href'] ?>" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200
                ...
                <span class="w-5 h-5 flex items-center justify-center shrink-0 transition-transform duration-200 hover:scale-110"><?= $item['icon'] ?></span> <span><?= $item['label'] ?></span>
            </a>
        <?php endforeach; ?>
    </nav>
    <div class="p-3 border-t border-teal-700/40">
        <a href="../logout.php" class="flex items-center gap-3 px-3 py-2.5 text-sm text-red-300 font-semibold rounded-lg transition-all duration-200 hover:bg-red-500/20 hover:text-white">
            <span class="w-5 h-5 flex items-center justify-center shrink-0">🚪</span> <span>Logout</span>
        </a>
    </div>
</aside>
