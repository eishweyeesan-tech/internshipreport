...
$total_weeks_count = count($weeks);
$progress_pct = $total_weeks_count > 0 ? min(round(($progress_weeks_completed / $total_weeks_count) * 100), 100) : 0;

// Total expected days & Overall Attendance rate (75% threshold)
...
if ($total_expected_days < 1) {
    $total_expected_days = max(1, $total_weeks_count * 5);
}
...
<body class="bg-slate-100 font-sans antialiased p-4 sm:p-6 md:p-8">
    ...
    <div class="max-w-7xl mx-auto w-full space-y-6">
        ...
        <!-- ════ STATS & ATTENDANCE CARDS (EXACT MATCH TO STUDENT DASHBOARD) ════ -->
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 no-print">
            <!-- Card 1: Total Hours Worked -->
            <div class="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 transition-all duration-300">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <span class="text-xs font-bold text-emerald-600 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-200">Logged</span>
                </div>
            ...
            <div class="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 transition-all duration-300">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <span class="text-xs font-bold text-blue-600 bg-blue-50 px-2.5 py-0.5 rounded-full border border-blue-200">Entries</span>
                </div>
            ...
            <!-- Card 3: Attendance Rate (75% Threshold Target) -->
            <div class="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 transition-all duration-300">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <span class="text-xs font-bold <?= $overall_attendance_rate >= 75 ? 'text-emerald-600 bg-emerald-50 border border-emerald-200' : 'text-amber-600 bg-amber-50 border border-amber-200' ?> px-2.5 py-0.5 rounded-full"><?= $overall_attendance_rate >= 75 ? 'Good' : 'Needs Attention' ?></span>
                </div>
                <p class="text-2xl font-black text-slate-800"><?= $overall_attendance_rate ?>%</p>
                <p class="text-xs text-slate-400 font-medium mt-1"><?= $total_present ?> / <?= $total_expected_days ?> days attended</p>
            </div>

            <!-- Card 4: Weeks Completed -->
            <div class="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 transition-all duration-300">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <span class="text-xs font-bold text-purple-600 bg-purple-50 px-2.5 py-0.5 rounded-full border border-purple-200">Weeks</span>
                </div>
                <p class="text-2xl font-black text-slate-800"><?= $progress_weeks_completed ?><span class="text-base font-bold text-slate-400">/<?= $total_weeks_count ?></span></p>
                <p class="text-xs text-slate-400 font-medium mt-1">Weeks Completed</p>
            </div>
        </div>

        <!-- ════ INTERNSHIP PROGRESS (EXACT MATCH TO STUDENT DASHBOARD) ════ -->
        <div class="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 no-print">
            <div class="flex items-center justify-between mb-3">
                ...
                <span class="text-xs font-bold text-indigo-600"><?= $progress_pct ?>% Completed</span>
            </div>
            ...
            <div class="flex items-center justify-between text-xs text-slate-500 flex-wrap gap-2">
                <div class="flex items-center gap-3">
                    <span class="inline-flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-indigo-500"></span> Weeks Logged: <strong class="text-slate-800"><?= $progress_weeks_completed ?>/<?= $total_weeks_count ?></strong></span>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 no-print">
            <div class="flex items-center justify-between flex-wrap gap-4">
                ...
                <!-- Save as PDF Action (Real-world Official Format) -->
                <a href="student/print_report.php?student_id=<?= $uid ?><?= $filter_week ? '&week=' . $filter_week : '&week=all' ?>" target="_blank" class="flex items-center gap-2 px-3.5 py-1.5 bg-[#005f73] hover:bg-[#0a9396] text-white rounded-xl text-xs font-bold transition shadow-xs cursor-pointer">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-6">
                        ...
                        <div class="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between flex-wrap gap-2 bg-gradient-to-r from-slate-50 to-white">
                            ...
                            <div class="flex items-center gap-2 flex-wrap">
                                <a href="student/print_report.php?student_id=<?= $uid ?>&week=<?= $wn ?>" target="_blank" class="inline-flex items-center gap-1.5 text-xs font-bold text-[#005f73] bg-teal-50 hover:bg-teal-100 border border-teal-200/80 px-2.5 py-1 rounded-lg transition shadow-2xs" title="Save official report as PDF for Week <?= $wn ?>">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                    </svg>
                                    <span>Save as PDF (W<?= $wn ?>)</span>
                                </a>
                                ...
                                <div class="border-t border-slate-100 pt-4">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                        <div class="bg-slate-50 rounded-xl p-3.5 border border-slate-100">
                                            <span class="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">What was done? <span class="text-slate-400 font-normal">/ ဘာလုပ်သလဲ</span></span>
                                            <p class="text-sm text-slate-700 leading-relaxed"><?= nl2br(htmlspecialchars($week_ref['what_done'] ?? '')) ?></p>
                                        </div>
                                        <div class="bg-slate-50 rounded-xl p-3.5 border border-slate-100">
                                            <span class="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">How was it done? <span class="text-slate-400 font-normal">/ ဘယ်လိုလုပ်ပါသလဲ</span></span>
                                            <p class="text-sm text-slate-700 leading-relaxed"><?= nl2br(htmlspecialchars($week_ref['how_done'] ?? '')) ?></p>
                                        </div>
                                        <div class="bg-slate-50 rounded-xl p-3.5 border border-slate-100">
                                            <span class="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">Why was it done? <span class="text-slate-400 font-normal">/ ဘာကြောင့်လုပ်ပါသလဲ</span></span>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                                    ...
                                    <div class="rounded-xl border border-slate-200 bg-gradient-to-b from-slate-50/70 to-white p-3.5 flex flex-col">
                                        <div class="flex items-center gap-2.5 mb-2">
                                            ...
                                            <?php if ($week_eval && $week_eval['report_status'] !== 'pending'): ?>
                                                <?php $ig = $week_eval['grade'] ?? '';
                                                $igd = $gmap[$ig] ?? ['—', 'text-slate-400', 'bg-slate-50']; ?>
                                    ...
                                    <div class="rounded-xl border border-slate-200 bg-gradient-to-b from-slate-50/70 to-white p-3.5 flex flex-col">
                                        <div class="flex items-center gap-2.5 mb-2">
                                            ...
                                            <?php if (!empty($week_sup_eval['weekly_grade'])): ?>
                                                <?php $sg = $week_sup_eval['weekly_grade'];
                                                $sgdi = $sgd[$sg] ?? [$sg, 'text-slate-700 bg-slate-50']; ?>
                                    ...
                                    <div class="px-4 py-2.5 bg-slate-50/90 flex items-center justify-between border-b border-slate-100">
                                        ...
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            foreach ($week_logs_m as $wl) {
                                                if ($wl['attendance_status'] === 'present') $wk_p++;
                                                else $wk_a++;
                                            }
                                            ?>
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3 px-4 pb-3">
                                            ...
                                            <div class="rounded-xl border border-slate-200 bg-gradient-to-b from-slate-50/70 to-white p-3.5 flex flex-col">
                                                <div class="flex items-center gap-2.5 mb-2">
                                                    ...
                                                    <?php if ($week_eval_m && $week_eval_m['report_status'] !== 'pending'): ?>
                                                        <?php $ig_m = $week_eval_m['grade'] ?? '';
                                                        $igd_m = $gmap[$ig_m] ?? ['—', 'text-slate-400', 'bg-slate-50']; ?>
                                            ...
                                            <div class="rounded-xl border border-slate-200 bg-gradient-to-b from-slate-50/70 to-white p-3.5 flex flex-col">
                                                <div class="flex items-center gap-2.5 mb-2">
                                                    ...
                                                    <?php if (!empty($week_sup_eval_m['weekly_grade'])): ?>
                                                        <?php $sg_m = $week_sup_eval_m['weekly_grade'];
                                                        $sgdi_m = $sgd[$sg_m] ?? [$sg_m, 'text-slate-700 bg-slate-50']; ?>
