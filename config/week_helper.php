...
/**
 * ══════════════════════════════════════════════════════════════════════
 *  WEEK HELPER — Internship Week Calculator Demo
 *  Demonstrates all week helper functions with mysqli, Tailwind CSS
 * ══════════════════════════════════════════════════════════════════════
 */
...
// ─── Fetch internship dates from database ────────────────────────────
$esc_uid = $conn->real_escape_string($student_id);
$profile_r = $conn->query("SELECT internship_start_date, internship_end_date FROM student_profiles WHERE user_id = {$esc_uid} LIMIT 1");

$internship_start = null;
$internship_end   = null;

if ($profile_r && $profile_r->num_rows > 0) {
    $row = $profile_r->fetch_assoc();
    $internship_start = $row['internship_start_date'] ?: null;
    $internship_end   = $row['internship_end_date'] ?: null;
    $profile_r->free();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $internship_start) {

    $action = $_POST['action'] ?? '';

    if ($action === 'week_number') {
        $test_date = trim($_POST['test_date'] ?? '');
        if (empty($test_date)) {
            $action_error = 'Please select a date.';
        } else {
            $wk = getInternshipWeekNumber($internship_start, $test_date);
            if ($wk === 0) {
                $action_error = "The date {$test_date} is before the internship start date ({$internship_start}).";
            } else {
                $action_result = [
                    'type' => 'week_number',
                    'date' => $test_date,
    ...
    if ($action === 'validate_date') {
        ...
        if (empty($test_date)) {
            ...
            if ($err) {
                $action_error = "The date {$test_date} is OUT OF RANGE ({$internship_start} to {$internship_end}).";
            } else {
                $action_result = [
                    'type' => 'validate_date',
                    'date' => $test_date,
    ...
    if ($action === 'week_range') {
        $test_week = (int) ($_POST['test_week'] ?? 0);
        if ($test_week < 1) {
            $action_error = 'Please enter a valid week number (1 or more).';
        } else {
            ...
            if ($range) {
                ...
                $action_error = "Could not compute range for week {$test_week}.";
            }
        }
    }
}

// ─── Build all weeks for display ─────────────────────────────────────
$weeks = [];
if ($internship_start) {
...
$current_week = 0;
if ($internship_start) {
    $current_week = getInternshipWeekNumber($internship_start, date('Y-m-d'));
}
